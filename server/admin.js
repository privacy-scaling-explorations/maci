...
const updateSubparser = (subparsers) => {
  const createParser = subparsers.addParser("update", { addHelp: true });
  createParser.addArgument(["-f", "--file_record"], {
    required: true,
    type: "string",
    help: "file location of deployed contract addresses",
  });
  createParser.addArgument(["-p", "--process_messages_zkey"], {
    action: "store",
    type: "string",
    help: "The .zkey file for the message processing circuit. ",
  });
  createParser.addArgument(["-t", "--tally_votes_zkey"], {
    action: "store",
    type: "string",
...
const removeSubparser = (subparsers) => {
  const createParser = subparsers.addParser("delete", { addHelp: true });
  createParser.addArgument(["-x", "--maci_addr"], {
    required: true,
    type: "string",
...
const querySubparser = (subparsers) => {
  const createParser = subparsers.addParser("query", { addHelp: true });
  createParser.addArgument(["-x", "--maci_addr"], {
    required: true,
    type: "string",
