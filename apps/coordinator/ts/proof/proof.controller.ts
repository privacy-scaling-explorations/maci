...
import { mapErrorToHttpStatus } from "../common/http.js";
...
export class ProofController {
  ...
  @ApiResponse({ status: HttpStatus.CONFLICT, description: "Operation cannot proceed due to state conflict (e.g., poll already tallied or trees not merged)" })
  @ApiResponse({ status: HttpStatus.INTERNAL_SERVER_ERROR, description: "Proof generation failed (encryption/decryption or merge failure)" })
  @Post("generate")
  ...
  @ApiResponse({ status: HttpStatus.CONFLICT, description: "Operation cannot proceed due to state conflict (e.g., trees not ready/merged)" })
  @ApiResponse({ status: HttpStatus.INTERNAL_SERVER_ERROR, description: "Merge failed (failed to merge state or message trees)" })
  @Post("merge")
  ...
  @ApiResponse({ status: HttpStatus.FORBIDDEN, description: "Invalid approval signature" })
  @ApiResponse({ status: HttpStatus.BAD_REQUEST, description: "Invalid input" })
  @ApiResponse({ status: HttpStatus.CONFLICT, description: "Operation cannot proceed due to state conflict (e.g., poll already tallied)" })
  @ApiResponse({ status: HttpStatus.INTERNAL_SERVER_ERROR, description: "On-chain submission failed (verifying keys/contract interaction errors)" })
  @Post("submit")
