...
export class DeployerController {
  ...
  @ApiResponse({ status: HttpStatus.INTERNAL_SERVER_ERROR, description: "Deployment failed or configuration error (e.g., RPC URL not set, contract deployment failed)" })
  @Post("maci")
  ...
  @ApiResponse({ status: HttpStatus.FORBIDDEN, description: "Invalid approval signature" })
  @ApiResponse({ status: HttpStatus.BAD_REQUEST, description: "Unsupported input (network/policy/voice credit proxy)" })
  @ApiResponse({ status: HttpStatus.PRECONDITION_FAILED, description: "Required contracts not deployed (MACI/Verifier/VerifyingKeysRegistry)" })
  @ApiResponse({ status: HttpStatus.INTERNAL_SERVER_ERROR, description: "Poll deployment failed (failed to deploy poll/set verifying keys/set MACI instance on policy)" })
  @Post("poll")
