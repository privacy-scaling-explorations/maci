...
class PubKey {
  ...
  public serialize = (): string => {
    ...
    if (BigInt(`${this.rawPubKey[0]}`) === BigInt(0) && BigInt(`${this.rawPubKey[1]}`) === BigInt(0)) {
...
class Ballot {
  ...
  public static genRandomBallot(_numVoteOptions: number, _voteOptionTreeDepth: number) {
    const ballot = new Ballot(_numVoteOptions, _voteOptionTreeDepth);
    ballot.nonce = genRandomSalt();
    return ballot;
  }

  public static genBlankBallot(_numVoteOptions: number, _voteOptionTreeDepth: number) {
    const ballot = new Ballot(_numVoteOptions, _voteOptionTreeDepth);
    return ballot;
  }
...
class StateLeaf implements IStateLeaf {
  ...
  static fromJSON(json: any): StateLeaf {
    return new StateLeaf(PubKey.unserialize(json.pubKey), BigInt(json.voiceCreditBalance), BigInt(json.timestamp));
  }
}
...
abstract class Command {
  ...
  static fromJSON(json: unknown): Command {
...
class TCommand extends Command {
  ...
  public equals = (command: TCommand): boolean => {
    return this.stateIndex === command.stateIndex && this.amount === command.amount;
  };
  ...
  static fromJSON(json: any): TCommand {
    const command = new TCommand(BigInt(json.stateIndex), BigInt(json.amount));
