...
import { StateLeaf, PCommand, Keypair, PrivKey, PubKey, VerifyingKey } from "../";
...
describe("Domain objects", function () {
  ...
  const random50bitBigInt = (): bigint => {
    return ((BigInt(1) << BigInt(50)) - BigInt(1)) & BigInt(`${genRandomSalt()}`);
  };
  ...
  describe("State leaves", () => {
    it("The serialize() and unserialize() functions should work correctly", () => {
      ...
      expect(unserialized.voiceCreditBalance.toString()).to.eq(stateLeaf.voiceCreditBalance.toString());
    });
  });
