...
describe("Cryptographic operations", function () {
    ...
    describe("ECDH shared key generation", () => {
        it("The shared keys should match", () => {
            expect(ecdhSharedKey[0].toString()).to.eq(
                ecdhSharedKey1[0].toString()
            );
            expect(ecdhSharedKey[1].toString()).to.eq(
                ecdhSharedKey1[1].toString()
            );
        });
    ...
    describe("Encryption and decryption", () => {
        it("The ciphertext should be of the correct format", () => {
            const expectedLength =
                plaintext.length <= 3 ? 4 : 1 + (plaintext.length % 3) * 3;
    ...
    describe("genTreeCommitment", () => {
        ...
        it("should generate a commitment to the tree root using the provided salt", () => {
            ...
            expect(commitment).to.satisfy(
                (num: bigint) => num < SNARK_FIELD_SIZE
            );
        });
