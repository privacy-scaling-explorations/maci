...
describe('BatchProcessMessage', () => {
    ...
    describe('Process messages', () => {
        it('the blank state leaf hash should match the one generated by the contract', async () => {
            const temp = new IncrementalQuinTree(voteOptionTreeDepth, BigInt(0), 5, hash5)
        ...
        it('batchProcessMessage should verify a proof and update the stateRoot', async () => {
            ...
            for (const p of batchUstCircuitInputs.circuitInputs['ecdh_public_key']) {
            ...
            const batchUstR = await genBatchUstProofAndPublicSignals(
                batchUstCircuitInputs.circuitInputs,
                config.env,
            )

            // Get the circuit-generated root
            //const circuitNewStateRoot = getSignalByName(circuit, witness, 'main.root')
            const circuitNewStateRoot = getSignalByNameViaSym('batchUst', batchUstR.witness, 'main.root')
    ...
    describe('Tally votes', () => {
        ...
        it('should tally a batch of votes', async () => {
            const r = await testTally(maciState, maciContract)
            tally = r.tally
            newResultsSalt = r.newResultsSalt
        })
    ...
    describe('Coordinator reset', () => {
        it('Should reset the correct storage variables', async () => {
            ...
            const currentQvtBatchNum = await maciContract.currentQvtBatchNum()
            const currentSpentVoiceCreditsCommitment = await maciContract.currentSpentVoiceCreditsCommitment()
            const currentResultsCommitment = await maciContract.currentResultsCommitment()
            const currentPerVOSpentVoiceCreditsCommitment = await maciContract.currentPerVOSpentVoiceCreditsCommitment()
            const totalVotes = await maciContract.totalVotes()

            expect(hasUnprocessedMessages).toBeTruthy()
            expect(stateRoot.toString()).toEqual(
                (await maciContract.stateRootBeforeProcessing()).toString()
            )

            const numMessages = await maciContract.numMessages()
            expect(currentMessageBatchIndex.toString()).toEqual(
                (Math.floor(numMessages / batchSize) * batchSize).toString()
            )

            expect(currentQvtBatchNum.toString()).toEqual('0')

            const originalSpentVoiceCreditsCommitment
                = await maciContract.originalSpentVoiceCreditsCommitment()

            expect(currentSpentVoiceCreditsCommitment.toString()).toEqual(
                originalSpentVoiceCreditsCommitment.toString(),
            )

            const originalCurrentResultsCommitment
                = await maciContract.originalCurrentResultsCommitment()

            expect(currentPerVOSpentVoiceCreditsCommitment.toString()).toEqual(
                originalCurrentResultsCommitment.toString(),
            )

            expect(totalVotes.toString()).toEqual('0')

            const temp = new IncrementalQuinTree(voteOptionTreeDepth, BigInt(0), 5, hash5)
            const blankResultsCommitment = hashLeftRight(temp.root, BigInt(0))
            expect(currentResultsCommitment.toString()).toEqual(blankResultsCommitment.toString())
        })

        it('Process again', async () => {
            const maciState = new MaciState(
                coordinator,
                stateTreeDepth,
                messageTreeDepth,
            ...
            const batchUstCircuitInputs = r.circuitInputs
            ...
            const ecdhPubKeys: PubKey[] = []
            for (const p of batchUstCircuitInputs['ecdh_public_key']) {
                const pubKey = new PubKey(p)
                ecdhPubKeys.push(pubKey)
            }
            ...
            const batchUstR = await genBatchUstProofAndPublicSignals(batchUstCircuitInputs, config.env)
        ...
        it('tally again', async () => {
            await testTally(maciState, maciContract)
        })
...
const testTally = async (
    ...
    const qvtR = await genQvtProofAndPublicSignals(circuitInputs, config.env)
    ...
    const newResultsCommitmentOutput = getSignalByNameViaSym('qvt', qvtR.witness, 'main.newResultsCommitment')
    ...
    const newSpentVoiceCreditsCommitmentOutput = getSignalByNameViaSym('qvt', qvtR.witness, 'main.newSpentVoiceCreditsCommitment')
    ...
    const newPerVOSpentVoiceCreditsCommitmentOutput = getSignalByNameViaSym('qvt', qvtR.witness, 'main.newPerVOSpentVoiceCreditsCommitment')
    ...
    const publicSignals = qvtR.publicSignals
