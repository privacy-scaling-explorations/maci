...
describe('MACI', () => {
    ...
    describe('Process messages', () => {
        let blankStateLeaf
        let blankStateLeafHash

        beforeAll(() => {
            blankStateLeaf = StateLeaf.genFreshLeaf(
                new PubKey([0, 0]),
                emptyVoteOptionTreeRoot,
                bigInt(0),
            )

            blankStateLeafHash = blankStateLeaf.hash()
        })
        ...
        it('the blank state leaf hash should match the one generated by the contract', async () => {
            ...
            expect(onChainHash.toString()).toEqual(blankStateLeafHash.toString())
        })
