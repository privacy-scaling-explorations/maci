/*
This test goes simulates the following scenario:

There is a coordinator and a user
The user owns a SignUpToken
The user signs up
...
describe('MACI', () => {
    ...
    describe('Process messages', () => {
        ...
        it('batchProcessMessage should verify a proof and update the postSignUpStateRoot', async () => {
            ...
            // No need to pad the on-chain state tree as its zero value is
            // already blankStateLeaf.hash(). But we do have to pass the blank
            // state leaves into the snark.
    ...
    describe('Tally votes', () => {
        ...
        it('the inputs and witness should be valid', async () => {
            ...
            for (let i = 0; i < batchSize; i++) {
                batchTree.insert(stateLeaves[i].hash())
            }
