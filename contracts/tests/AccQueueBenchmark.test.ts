...
const deploy = async (
    ...
    aqContract = await AccQueueFactory.deploy(SUB_DEPTH);
...
const testMerge = async (
    ...
    for (let i = 0; i < NUM_SUBTREES; i++) {
        ...
        await (
            await aqContract.enqueue(leaf.toString(), { gasLimit: 200000 })
        ).wait();
        aq.enqueue(leaf);
    ...
    if (NUM_MERGES === 0) {
        ...
        console.log(
            `mergeSubRoots() for ${NUM_SUBTREES} subtrees: ${receipt.gasUsed.toString()} gas`
        );
    } else {
    ...
    receipt = await tx.wait();
    console.log(
        `merge() for ${NUM_SUBTREES} subtrees to depth ${MAIN_DEPTH}: ${receipt.gasUsed.toString()} gas`
    );
    ...
    describe("Binary enqueues", () => {
        ...
        it(`Should enqueue to a subtree of depth ${SUB_DEPTH}`, async () => {
            for (let i = 0; i < HASH_LENGTH ** SUB_DEPTH; i++) {
                ...
                console.log(
                    `Gas used by binary enqueue: ${receipt.gasUsed.toString()}`
                );
            }
        });
    });
    ...
    describe("Binary fills", () => {
        ...
        it(`Should fill to a subtree of depth ${SUB_DEPTH}`, async () => {
            for (let i = 0; i < 2; i++) {
                ...
                const receipt = await tx.wait();
                console.log(
                    `Gas used by binary fill: ${receipt.gasUsed.toString()}`
                );
            }
        });
    });
    ...
    describe("Quinary enqueues", () => {
        ...
        it(`Should enqueue to a subtree of depth ${SUB_DEPTH}`, async () => {
            for (let i = 0; i < HASH_LENGTH ** SUB_DEPTH; i++) {
                ...
                const receipt = await tx.wait();
                console.log(
                    `Gas used by quinary enqueue: ${receipt.gasUsed.toString()}`
                );
            }
        });
    });
    ...
    describe("Quinary fills", () => {
        ...
        it(`Should fill a subtree of depth ${SUB_DEPTH}`, async () => {
            for (let i = 0; i < 2; i++) {
                ...
                const receipt = await tx.wait();
                console.log(
                    `Gas used by quinary fill: ${receipt.gasUsed.toString()}`
                );
            }
        });
    });
