...
describe("MACI", () => {
    ...
    describe("Signups", () => {
        it("should sign up users", async () => {
            ...
            for (const user of users) {
                ...
                expect(event.args._stateIndex.toString()).to.eq(
                    (i + 1).toString()
                );
    ...
    describe("Deploy a Poll", () => {
        ...
        it("should set correct storage values", async () => {
            ...
            expect(Number(onChainMaxValues.maxMessages)).to.eq(
                maxValues.maxMessages
            );
            expect(Number(onChainMaxValues.maxVoteOptions)).to.eq(
                maxValues.maxVoteOptions
            );

            const onChainTreeDepths = await pollContract.treeDepths();
            expect(Number(onChainTreeDepths.intStateTreeDepth)).to.eq(
                treeDepths.intStateTreeDepth
            );
            expect(Number(onChainTreeDepths.messageTreeDepth)).to.eq(
                treeDepths.messageTreeDepth
            );
            expect(Number(onChainTreeDepths.messageTreeSubDepth)).to.eq(
                treeDepths.messageTreeSubDepth
            );
            expect(Number(onChainTreeDepths.voteOptionTreeDepth)).to.eq(
                treeDepths.voteOptionTreeDepth
            );

            const onChainBatchSizes = await pollContract.batchSizes();
            expect(Number(onChainBatchSizes.messageBatchSize)).to.eq(
                messageBatchSize
            );
            expect(Number(onChainBatchSizes.tallyBatchSize)).to.eq(
                tallyBatchSize
            );
        });
    });
    ...
    describe("Merge messages", () => {
        ...
        it("the message root must be correct", async () => {
            const onChainMessageRoot =
                await messageAqContract.getMainRoot(MESSAGE_TREE_DEPTH);
    ...
    describe("Merge sign-ups as the Poll", () => {
        ...
        it("the state root must be correct", async () => {
            const onChainStateRoot =
                await stateAqContract.getMainRoot(STATE_TREE_DEPTH);
            expect(onChainStateRoot.toString()).to.eq(
    ...
    describe("Process messages", () => {
        ...
        it("genProcessMessagesPackedVals() should generate the correct value", async () => {
            ...
            expect(packedVals.toString(16)).to.eq(
                onChainPackedVals.toString(16)
            );
        });
        ...
        it("processMessages() should update the state and ballot root commitment", async () => {
            ...
            expect(generatedInputs.newSbCommitment).to.eq(
    ...
    describe("Tally votes", () => {
        ...
        it("tallyVotes() should update the tally commitment", async () => {
            ...
            expect(generatedInputs.newTallyCommitment).to.eq(
