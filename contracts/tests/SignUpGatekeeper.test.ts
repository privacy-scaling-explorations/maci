...
describe("SignUpGatekeeper", () => {
    ...
    describe("Deployment", () => {
        ...
        it("SignUpTokenGatekeeper has token address set", async () => {
            expect(await signUpTokenGatekeeperContract.token()).to.eq(
                await signUpToken.address
            );
        });
    });
    ...
    describe("SignUpTokenGatekeeper", () => {
        ...
        it("sets MACI instance correctly", async () => {
            await signUpTokenGatekeeperContract.setMaciInstance(
                await maciContract.address
            );

            expect(await signUpTokenGatekeeperContract.maci()).to.eq(
                await maciContract.address
            );
        });
