...
const testFillForAllIncompletes = async (
    ...
    for (let i = 0; i < HASH_LENGTH; i++) {
        for (let j = 0; j < i; j++) {
            ...
            aq.enqueue(leaf);
            await (
                await aqContract.enqueue(leaf.toString(), enqueueGasLimit)
            ).wait();
        }
        aq.fill();
        await (await aqContract.fill(fillGasLimit)).wait();
...
const testEmptyUponDeployment = async (aqContract: any) => {
    ...
    await expect(aqContract.getSubRoot(0)).to.be.revertedWithCustomError(
        aqContract,
        "InvalidIndex"
    );
};
...
const testInsertSubTrees = async (
    ...
    for (let i = 0; i < NUM_SUBTREES; i++) {
        ...
        await (
            await aqContract.insertSubTree(
                subTree.root.toString(),
                insertSubTreeGasLimit
            )
        ).wait();
    }
    ...
    expect(expectedSmallSRTroot.toString()).to.eq(
        contractSmallSRTroot.toString()
    );
 ...
 */
const testEnqueueAndInsertSubTree = async (aq: AccQueue, aqContract: any) => {
    const z = aq.zeros[0];
    const n = BigInt(1);
...
const testMerge = async (
    ...
    await (await aqContract.merge(MAIN_DEPTH, { gasLimit: 8000000 })).wait();
    const contractMainRoot = (
        await aqContract.getMainRoot(MAIN_DEPTH)
    ).toString();
...
describe("AccQueues", () => {
    describe("Binary AccQueue enqueues", () => {
        ...
        it("Should not be able to get a subroot that does not exist", async () => {
            await expect(
                aqContract.getSubRoot(0)
            ).to.be.revertedWithCustomError(aqContract, "InvalidIndex");
        });
    ...
    describe("Edge cases", () => {
        ...
        it("Should not be possible to merge if empty", async () => {
            ...
            const aqContract = r.aqContract;
            await expect(
                aqContract.mergeSubRoots(0, { gasLimit: 1000000 })
            ).to.be.revertedWithCustomError(aqContract, "NothingToMerge");
        });
        ...
        it("Should not be possible to merge into a tree of depth 0", async () => {
            ...
            await (
                await aqContract.mergeSubRoots(0, { gasLimit: 1000000 })
            ).wait();
            await expect(
                aqContract.merge(0, { gasLimit: 1000000 })
            ).to.revertedWithCustomError(aqContract, "DepthCannotBeZero");
        });
        ...
        it("Should not be possible to merge subroots into a tree shorter than the SRT depth", async () => {
            ...
            await expect(
                aqContract.merge(1, { gasLimit: 1000000 })
            ).to.be.revertedWithCustomError(aqContract, "DepthTooSmall");
        });
        ...
        it("Merging without enqueing new data should not change the root", async () => {
            ...
            // Merge again
            await (
                await aqContract.merge(MAIN_DEPTH, { gasLimit: 8000000 })
            ).wait();
            // Get the root again
    ...
    describe("Binary AccQueue0 progressive merges", () => {
        ...
        it(`Should progressively merge ${NUM_SUBTREES} subtrees`, async () => {
            ...
            await expect(
                aqContract.getSmallSRTroot()
            ).to.be.revertedWithCustomError(aqContract, "SubTreesNotMerged");
            ...
            expect(expectedMainRoot.toString()).to.eq(
                contractMainRoot.toString()
            );
        });
    });
    ...
    describe("Conditions that cause merge() to revert", () => {
        ...
        it(`merge() should revert if the desired depth is invalid`, async () => {
            ...
            await expect(
                aqContract.merge(0, { gasLimit: 1000000 })
            ).to.be.revertedWithCustomError(aqContract, "DepthCannotBeZero");
        });
    });
});
