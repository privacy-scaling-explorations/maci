...
contract Utilities is SnarkConstants, Hasher, IPubKey, IMessage {
  ...
  function hashMessageAndEncPubKey(Message memory _message, PubKey memory _encPubKey) public pure returns (uint256) {
    ...
    return hash5([_message.msgType, hash5(n), hash5(m), _encPubKey.x, _encPubKey.y]);
  }
}
