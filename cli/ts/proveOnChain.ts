...
import { compareOnChainValue, validateEthAddress, contractExists, delay } from "./utils";
...
const proveOnChain = async (args: any) => {
  ...
  const maciContract = new ethers.Contract(maciAddress, maciContractAbi, signer);
  ...
  const tallyContract = new ethers.Contract(tallyAddress, tallyContractAbi, signer);

  const subsidyContract = new ethers.Contract(subsidyAddress, subsidyContractAbi, signer);
  ...
  const verifierContract = new ethers.Contract(verifierContractAddress, verifierContractAbi, signer);
  ...
  if (Object.keys(data.subsidyProofs).length !== 0) {
    ...
    for (let i = subsidyBatchNum; i < totalBatchNum; i++) {
      ...
      if (
        !compareOnChainValue("currentSbCommitment", currentSbCommitmentOnChain.toString(), circuitInputs.sbCommitment)
      ) {
        return;
      }
