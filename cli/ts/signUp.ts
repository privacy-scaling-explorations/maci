...
const configureSubparser = (subparsers: any) => {
  const parser = subparsers.addParser("signup", { addHelp: true });
  ...
  parser.addArgument(["-v", "--ivcp-data"], {
    action: "store",
    type: "string",
...
const signup = async (args: any) => {
  ...
  let tx;
  try {
    tx = await maciContract.signUp(userMaciPubKey.asContractParam(), sgData, ivcpData, { gasLimit: 1000000 });
  } catch (e) {
...
export { signup, configureSubparser };
