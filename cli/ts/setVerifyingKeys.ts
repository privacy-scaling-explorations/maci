...
const setVerifyingKeys = async (args: any) => {
  ...
  // Simple validation
  if (stateTreeDepth < 1 || intStateTreeDepth < 1 || msgTreeDepth < 1 || voteOptionTreeDepth < 1 || msgBatchDepth < 1) {
  ...
  const vkRegistryContract = new ethers.Contract(vkRegistryAddress, vkRegistryAbi, signer);

  const messageBatchSize = 5 ** msgBatchDepth;

  // Query the contract to see if the processVk has been set
  const processVkSig = genProcessVkSig(stateTreeDepth, msgTreeDepth, voteOptionTreeDepth, messageBatchSize);

  const isProcessVkSet = await vkRegistryContract.isProcessVkSet(processVkSig);
