...
const configureSubparser = (subparsers: any) => {
    const createParser = subparsers.addParser("setVerifyingKeys", {
        addHelp: true,
    });

    createParser.addArgument(["-k", "--vk_registry"], {
        action: "store",
        type: "string",
    ...
    createParser.addArgument(["-b", "--msg-batch-depth"], {
        action: "store",
        type: "int",
        required: true,
    ...
    createParser.addArgument(["-p", "--process-messages-zkey"], {
        action: "store",
        type: "string",
        required: true,
    ...
    createParser.addArgument(["-t", "--tally-votes-zkey"], {
        action: "store",
        type: "string",
        required: true,
    ...
    createParser.addArgument(["-ss", "--subsidy-zkey"], {
        action: "store",
        type: "string",
...
const setVerifyingKeys = async (args: any) => {
    ...
    if (stateTreeDepth < intStateTreeDepth) {
        console.error(
            "Error: invalid state tree depth or intermediate state tree depth"
        );
        return;
    }
    ...
    if (
        ...
        console.error(
            "Error: incorrect .zkey file; please check the circuit params"
        );
        return;
    }
    ...
    if (!(await contractExists(signer.provider, vkRegistryAddress))) {
        console.error(
            "Error: a VkRegistry contract is not deployed at",
            vkRegistryAddress
        );
        return;
    }
    ...
    const isProcessVkSet =
        await vkRegistryContract.isProcessVkSet(processVkSig);

    if (isProcessVkSet) {
        console.error(
            "Error: this process verifying key is already set in the contract"
        );
        return;
    }
    ...
    if (isTallyVkSet) {
        console.error(
            "Error: this tally verifying key is already set in the contract"
        );
        return;
    }
    ...
    if (args.subsidy_zkey) {
        ...
        const isSubsidyVkSet =
            await vkRegistryContract.isSubsidyVkSet(subsidyVkSig);
        if (isSubsidyVkSet) {
            console.error(
                "Error: this subsidy verifying key is already set in the contract"
            );
            return;
        }
    }
...
const compareVks = (vk: VerifyingKey, vkOnChain: any): boolean => {
    ...
    for (let i = 0; i < vk.ic.length; i++) {
        isEqual =
            isEqual && vk.ic[i].x.toString() === vkOnChain.ic[i].x.toString();
        isEqual =
            isEqual && vk.ic[i].y.toString() === vkOnChain.ic[i].y.toString();
    }
    isEqual =
        isEqual && vk.alpha1.x.toString() === vkOnChain.alpha1.x.toString();
    isEqual =
        isEqual && vk.alpha1.y.toString() === vkOnChain.alpha1.y.toString();
    isEqual =
    ...
    isEqual =
        isEqual &&
        vk.delta2.x[0].toString() === vkOnChain.delta2.x[0].toString();
    isEqual =
        isEqual &&
        vk.delta2.x[1].toString() === vkOnChain.delta2.x[1].toString();
    isEqual =
        isEqual &&
        vk.delta2.y[0].toString() === vkOnChain.delta2.y[0].toString();
    isEqual =
        isEqual &&
        vk.delta2.y[1].toString() === vkOnChain.delta2.y[1].toString();
    isEqual =
        isEqual &&
        vk.gamma2.x[0].toString() === vkOnChain.gamma2.x[0].toString();
    isEqual =
        isEqual &&
        vk.gamma2.x[1].toString() === vkOnChain.gamma2.x[1].toString();
    isEqual =
        isEqual &&
        vk.gamma2.y[0].toString() === vkOnChain.gamma2.y[0].toString();
    isEqual =
        isEqual &&
        vk.gamma2.y[1].toString() === vkOnChain.gamma2.y[1].toString();

    return isEqual;
};

export { setVerifyingKeys, configureSubparser, compareVks };
