...
export const genProofs = async (
    ...
    if (subsidyFile) {
        if (existsSync(subsidyFile))
            logError(
                `${subsidyFile} exists. Please specify a different filepath.`
            );
        if (!subsidyZkey)
    ...
    const maciPrivKey = PrivKey.unserialize(privateKey);
    ...
    if (stateFile) {
        ...
        try {
            ...
            for (const poll of maciState.polls) {
                poll.setCoordinatorKeypair(privateKey);
            }
        } catch (error: any) {
    ...
    if (subsidyFile) {
        ...
        while (poll.hasUnfinishedSubsidyCalculation()) {
            ...
            try {
                ...
                writeFileSync(
                    join(outputDir, `subsidy_${numBatchesCalulated}.json`),
                    JSON.stringify(thisProof, null, 4)
                );
                numBatchesCalulated++;
    ...
    while (poll.hasUntalliedBallots()) {
        ...
        try {
            ...
            writeFileSync(
                join(outputDir, `tally_${poll.numBatchesTallied - 1}.json`),
                JSON.stringify(thisProof, null, 4)
            );
