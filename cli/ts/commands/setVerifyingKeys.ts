...
export const setVerifyingKeys = async (
    ...
    const vkRegistryAddress = vkRegistry
        ? vkRegistry
        : readContractAddress("VkRegistry");

    // check if zKey files exist
    if (!existsSync(processMessagesZkeyPath))
        logError(`1 ${processMessagesZkeyPath} does not exist.`);
    if (!existsSync(tallyVotesZkeyPath))
        logError(`2 ${tallyVotesZkeyPath} does not exist.`);
    if (subsidyZkeyPath && !existsSync(subsidyZkeyPath))
        logError(`3 ${subsidyZkeyPath} does not exist.`);
    ...
    const pmMatch = processMessagesZkeyPath.match(
        /.+_(\d+)-(\d+)-(\d+)-(\d+)_/
    );
    if (!pmMatch)
    ...
    if (!(await contractExists(signer.provider, vkRegistryAddress)))
        logError(
            `A VkRegistry contract is not deployed at ${vkRegistryAddress}`
        );
    ...
    if (subsidyZkeyPath) {
        ...
        if (await vkRegistryContract.isSubsidyVkSet(subsidyVkSig))
            logError(
                "This subsidy verifying key is already set in the contract"
            );
    }
    ...
    try {
        ...
        if (subsidyZkeyPath) {
            const subsidyVk: VerifyingKey = VerifyingKey.fromObj(
                await extractVk(subsidyZkeyPath)
            );
