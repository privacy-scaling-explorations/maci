...
export const mergeSignups = async ({
    ...
    while (true) {
        ...
        if (subTreesMerged) {
            if (!quiet) logGreen(success('All state subtrees have been merged.'))
            break
        }
        ...
        if (!quiet) {
            ...
            logGreen(success(`Executed mergeMaciStateAqSubRoots(); gas used: ${receipt.gasUsed.toString()}`))
        }
    }
    ...
    if (mainRoot === '0' || pollId > 0) {
        ...
        if (!quiet) {
            logYellow(info(`Transaction hash: ${receipt.transactionHash}`))
            logGreen(success(`Executed mergeStateAq(); gas used: ${receipt.gasUsed.toString()}`))
        }
    } else {
