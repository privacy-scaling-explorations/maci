...
export const verify = async (
    ...
    if (!(await contractExists(signer.provider, maciContractAddress))) {
        logError(
            `Error: there is no contract deployed at ${maciContractAddress}.`
        );
    }
    if (!(await contractExists(signer.provider, tallyContractAddress))) {
        logError(
            `Error: there is no contract deployed at ${tallyContractAddress}.`
        );
    }
    if (!(await contractExists(signer.provider, subsidyContractAddress))) {
        logError(
            `Error: there is no contract deployed at ${subsidyContractAddress}.`
        );
    }
