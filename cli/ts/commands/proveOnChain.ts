...
export const proveOnChain = async (
    ...
    const tallyContractAddress = tallyAddress
        ? tallyAddress
        : readContractAddress("Tally-" + pollId);
    const subsidyContractAddress = subsidyAddress
        ? subsidyAddress
        : readContractAddress("Subsidy-" + pollId);
    ...
    if (!(await contractExists(signer.provider, pollAddr))) {
        logError(
            "There is no Poll contract with this poll ID linked to the specified MACI contract."
        );
    }
    ...
    if (!(await contractExists(signer.provider, messageAqContractAddress))) {
        logError(
            "There is no MessageAq contract linked to the specified MACI contract."
        );
    }
    ...
    if (!(await contractExists(signer.provider, vkRegistryContractAddress))) {
        logError(
            "There is no VkRegistry contract linked to the specified MACI contract."
        );
    }
    ...
    if (!(await contractExists(signer.provider, verifierContractAddress))) {
        logError(
            "There is no Verifier contract linked to the specified MACI contract."
        );
    }
    ...
    for (let i = numberBatchesProcessed; i < totalMessageBatches; i++) {
        ...
        if (numberBatchesProcessed === 0) {
            ...
            currentMessageBatchIndex =
                (totalMessageBatches - numberBatchesProcessed) *
                messageBatchSize;
