...
const configureSubparser = (subparsers: any) => {
  const parser = subparsers.addParser("mergeSignups", { addHelp: true });
...
const mergeSignups = async (args: any) => {
  ...
  const accQueueContract = new ethers.Contract(await maciContractEthers.stateAq(), accQueueContractAbi, signer);
  ...
  while (true) {
    ...
    const tx = await pollContract.mergeMaciStateAqSubRoots(args.num_queue_ops.toString(), pollId.toString());
...
export { mergeSignups, configureSubparser };
