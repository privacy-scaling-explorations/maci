...
import { promptPwd, validateEthAddress, validateSaltSize, validateSaltFormat, contractExists } from "./utils";
...
const configureSubparser = (subparsers: any) => {
  const parser = subparsers.addParser("publish", { addHelp: true });

  parser.addArgument(["-p", "--pubkey"], {
    required: true,
    type: "string",
  ...
  parser.addArgument(["-i", "--state-index"], {
    required: true,
    action: "store",
    type: "int",
  ...
  parser.addArgument(["-v", "--vote-option-index"], {
    required: true,
    action: "store",
    type: "int",
  ...
  parser.addArgument(["-w", "--new-vote-weight"], {
    required: true,
    action: "store",
    type: "int",
...
const publish = async (args: any) => {
  ...
  const pollContractEthers = new ethers.Contract(pollAddr, pollContractAbi, signer);
  ...
  let tx: { wait: () => any; hash: any };
  try {
    tx = await pollContractEthers.publishMessage(message.asContractParam(), encKeypair.pubKey.asContractParam(), {
      gasLimit: 10000000,
    });
    await tx.wait();
