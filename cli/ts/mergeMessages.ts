...
import { validateEthAddress, contractExists, currentBlockTimestamp } from "./utils";
import { readJSONFile } from "maci-common";

import * as ethers from "ethers";

import { DEFAULT_SR_QUEUE_OPS } from "./defaults";
import { contractFilepath } from "./config";

const configureSubparser = (subparsers: any) => {
  const parser = subparsers.addParser("mergeMessages", { addHelp: true });
  ...
  parser.addArgument(["-n", "--num-queue-ops"], {
    action: "store",
    type: "int",
...
const mergeMessages = async (args: any) => {
  ...
  const maciContractEthers = new ethers.Contract(maciAddress, maciContractAbi, signer);
  ...
  const accQueueContract = new ethers.Contract(messageAqContractAddr, accQueueContractAbi, signer);
  ...
  if (pollOwner.toLowerCase() !== signer.address.toLowerCase()) {
    console.error("Error: the signer is not the owner of this Poll contract " + pollAddr);
    return;
  }
  ...
  if (now < deadline) {
    console.error("Error: the voting period is not over. " + "Please wait till " + new Date(deadline * 1000));
    return;
  }
  ...
  while (true) {
    ...
    const indices = (await accQueueContract.getSrIndices()).map((x) => Number(x));

    console.log(`Merging message subroots ${indices[0] + 1} / ${indices[1] + 1}`);

    const tx = await pollContract.mergeMessageAqSubRoots(args.num_queue_ops.toString());
...
export { mergeMessages, configureSubparser };
