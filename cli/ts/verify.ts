...
import { compareOnChainValue, validateEthAddress, contractExists } from "./utils";
...
const verify = async (args: any) => {
  ...
  let validResultsCommitment = data.newTallyCommitment && data.newTallyCommitment.match(/0x[a-fA-F0-9]+/);
  ...
  if (args.subsidy_file) {
    ...
    validResultsCommitment = data.newSubsidyCommitment && data.newSubsidyCommitment.match(/0x[a-fA-F0-9]+/);
