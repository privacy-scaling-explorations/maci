...
import { promptPwd, validateEthAddress, contractExists, isPathExist } from "./utils";
...
const configureSubparser = (subparsers: any) => {
  ...
  parser.addArgument(["-st", "--state-file"], {
    action: "store",
    type: "string",
  ...
  parser.addArgument(["-sb", "--start-block"], {
    type: "int",
    help: "The starting block to fetch logs from",
  });

  parser.addArgument(["-eb", "--end-block"], {
    type: "int",
    help: "The last block to fetch logs from",
  });

  parser.addArgument(["-bb", "--blocks-per-batch"], {
    type: "int",
...
const genProofs = async (args: any) => {
  ...
  if (args.tally_witnessgen && args.process_witnessgen) {
    const [ok, path] = isPathExist([rapidsnarkExe, args.process_witnessgen + ".dat", args.tally_witnessgen + ".dat"]);
    if (!ok) {
  ...
  if (args.subsidy_file) {
    ...
    if (args.subsidy_witnessgen) {
      ...
      const [ok, path] = isPathExist([args.subsidy_witnessgen, subsidyDatFile, args.subsidy_zkey]);
      if (!ok) {
        console.error(`Error: ${path} does not exist.`);
        return;
      }
    } else {
      const [ok, path] = isPathExist([args.subsidy_wasm, args.subsidy_zkey]);
      if (!ok) {
        console.error(`Error: ${path} does not exist.`);
        return;
      }
  ...
  const messageAqContract = new ethers.Contract(messageAqContractAddr, accQueueContractAbi, signer);
  ...
  if (mainRoot === "0") {
    console.error(
      "Error: the message tree has not been merged yet. " + "Please use the mergeMessages subcommmand to do so.",
    );
    return;
  }
...
const saveOutput = (outputDir: string, proof: any, filename: string) => {
  fs.writeFileSync(path.join(outputDir, filename), JSON.stringify(proof, null, 2));
};
