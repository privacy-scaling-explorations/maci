...
describe("MaciState", function () {
    ...
    describe("Process and tally 1 message from 1 user", () => {
        ...
        it("the state root should be correct", () => {
            ...
            expect(
                maciState.stateAq.getRoot(STATE_TREE_DEPTH).toString()
            ).to.eq(stateTree.root.toString());
        });
        ...
        it("the message root should be correct", () => {
            ...
            expect(
                maciState.polls[pollId].messageAq
                    .getRoot(treeDepths.messageTreeDepth)
                    .toString()
            ).to.eq(msgTree.root.toString());
        });
        ...
        it("packProcessMessageSmallVals and unpackProcessMessageSmallVals", () => {
            ...
            const unpacked =
                MaciState.unpackProcessMessageSmallVals(packedVals);
            expect(unpacked.maxVoteOptions.toString()).to.eq(
                maxVoteOptions.toString()
            );
            expect(unpacked.numUsers.toString()).to.eq(numUsers.toString());
            expect(unpacked.batchStartIndex.toString()).to.eq(
                batchStartIndex.toString()
            );
            expect(unpacked.batchEndIndex.toString()).to.eq(
                batchEndIndex.toString()
            );
        });
        ...
        it("Process a batch of messages (though only 1 message is in the batch)", () => {
            ...
            // Check the ballot
            expect(
                maciState.polls[pollId].ballots[1].votes[
                    Number(voteOptionIndex)
                ].toString()
            ).to.eq(voteWeight.toString());
            // Check the state leaf in the poll
            expect(
                maciState.polls[
                    pollId
                ].stateLeaves[1].voiceCreditBalance.toString()
            ).to.eq((voiceCreditBalance - voteWeight * voteWeight).toString());
        });
    ...
    describe(`Process and tally ${
        messageBatchSize * 2
    } messages from ${messageBatchSize} users`, () => {
        let maciState;
        let pollId;
        const voteWeight = BigInt(9);
        ...
        it("should process votes correctly", () => {
            ...
            expect(maciState.polls[pollId].messageAq.numLeaves).to.eq(
                messageBatchSize - 1
            );
            ...
            expect(maciState.polls[pollId].currentMessageBatchIndex).to.eq(
                undefined
            );
            ...
            expect(r.stateLeaves.length).to.eq(
                maciState.polls[pollId].stateLeaves.length
            );

            expect(r.ballots.length).to.eq(
                maciState.polls[pollId].ballots.length
            );
            ...
            for (let i = 0; i < r.stateLeaves.length; i++) {
                ...
                expect(r.ballots[i].equals(maciState.polls[pollId].ballots[i]))
                    .to.be.true;
            }
        });
        ...
        it("should tally ballots correctly", () => {
            ...
            for (
                let i = 0;
                i < maciState.polls[pollId].results.length - 1;
                i++
            ) {
                expect(maciState.polls[pollId].results[i].toString()).to.eq(
                    voteWeight.toString()
                );
            }
    ...
    describe('Deep copy', () => {
        ...
        it('should correctly deep-copy a MaciState object', () => {
            ...
            const m4 = m1.copy();
            m4.stateLeaves[0].voiceCreditBalance =
                BigInt(m4.stateLeaves[0].voiceCreditBalance) + BigInt(1);
            ...
            const m11 = m1.copy();
            m11.polls[pollId].treeDepths.messageTreeSubDepth =
            ...
            const m12 = m1.copy();
            m12.polls[pollId].treeDepths.voteOptionTreeDepth =
            ...
            const m13 = m1.copy();
            m13.polls[pollId].batchSizes.tallyBatchSize =
