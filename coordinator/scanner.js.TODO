...
const { decrypt, decryptAndVerify } = require('./utils/crypto')
...
const main = async () => {
  ...
  signUpLogs.forEach((signUpLog) => {
    const encryptedMessage = signUpLog.values.encryptedMessage.map(x => BigInt(x.toString()))
    const ecdhPublicKey = signUpLog.values.ecdhPublicKey.map(x => BigInt(x.toString()))
    const hashedEncryptedMessage = BigInt(signUpLog.values.hashedEncryptedMessage.toString())
    ...
    stateTree.insert(
      ...
      decryptedMessage.slice(0, 3)
    )

    if (cmdTree.root.toString() !== newTreeRoot) {
      throw new Error('Invalid tree root - cmdTree does not match new tree root!')
    }
  })

  // Voting process begins
  commandPublishedLogs.forEach((commandPublishedLog) => {
    const encryptedMessage = commandPublishedLog.values.encryptedMessage.map(x => BigInt(x.toString()))
    const ecdhPublicKey = commandPublishedLog.values.ecdhPublicKey.map(x => BigInt(x.toString()))
    const hashedEncryptedMessage = BigInt(commandPublishedLog.values.hashedEncryptedMessage.toString())
