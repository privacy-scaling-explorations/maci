...
const genSnarkVerifierSol = (contractName: string, vk: any) => {
  const templatePath = path.join(__dirname, "../ts/verifier_groth16.sol");

  let template = fs.readFileSync(templatePath).toString();

  template = template.replace("<%contract_name%>", contractName);

  const vkalpha1 = `uint256(${vk.vk_alfa_1[0].toString()}),` + `uint256(${vk.vk_alfa_1[1].toString()})`;
