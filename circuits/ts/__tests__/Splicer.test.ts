...
describe("Splice circuit", () => {
    let circuit: any;
    before(async () => {
        const circuitPath = join(__dirname, '../../circom/test', `splicer_test.circom`)
    ...
    it("Should output the correct reconstructed level", async () => {
        for (let index = 0; index < 5; index++) {
            ...
            expect(
                JSON.stringify(stringifyBigInts(items.map(BigInt)))
            ).to.be.eq(JSON.stringify(stringifyBigInts(output.map(String))));
        }
    });
});