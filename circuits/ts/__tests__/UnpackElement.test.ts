...
describe("UnpackElement circuit", () => {
    ...
    describe('UnpackElement', () => {
        before(async () => {
            const circuitPath = join(__dirname, '../../circom/test', `unpackElement_test.circom`)
        ...
        it("Should unpack a field element with 5 packed values correctly", async () => {
            ...
            for (let i = 0; i < 5; i++) {
                let e = (
                    BigInt(genRandomSalt().toString()) % BigInt(2 ** 50)
                ).toString(2);
                while (e.length < 50) {
                    e = "0" + e;
                }
            ...
            for (let i = 0; i < 5; i++) {
                ...
                expect(
                    BigInt("0b" + BigInt(out).toString(2)).toString()
    ...
    describe("unpackElement4", () => {
        before(async () => {
            const circuitPath = join(__dirname, '../../circom/test', `unpackElement4_test.circom`)
            circuit = await tester(circuitPath)
        })
