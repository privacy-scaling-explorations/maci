...
describe("StateLeafAndBallotTransformer circuit", function () {
    ...
    before(async () => {
        const circuitPath = path.join(
            __dirname,
            "../../circom/test",
            `stateLeafAndBallotTransformer_test.circom`
        );
    ...
    it("Should output new state leaf and ballot values if the command is valid", async () => {
        ...
        const newSlPubKey0 = await getSignal(
            circuit,
            witness,
            "newSlPubKey[0]"
        );
        const newSlPubKey1 = await getSignal(
            circuit,
            witness,
            "newSlPubKey[1]"
        );
        const newBallotNonce = await getSignal(
            circuit,
            witness,
            "newBallotNonce"
        );

        expect(newSlPubKey0.toString()).to.be.eq(
            command.newPubKey.rawPubKey[0].toString()
        );
        expect(newSlPubKey1.toString()).to.be.eq(
            command.newPubKey.rawPubKey[1].toString()
        );
    ...
    it("Should output existing state leaf and ballot values if the command is invalid", async () => {
        ...
        expect(newSlPubKey0.toString()).to.be.eq(
            slPubKey.rawPubKey[0].toString()
        );
        expect(newSlPubKey1.toString()).to.be.eq(
            slPubKey.rawPubKey[1].toString()
        );
