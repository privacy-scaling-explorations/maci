...
describe("ProcessMessage circuit", function () {
    ...
    before(async () => {
        const circuitPath = join(__dirname, '../../circom/test', `processMessages_test.circom`)
        circuit = await tester(circuitPath)
        const hasherCircuitPath = join(__dirname, '../../circom/test', `processMessagesInputHasher_test.circom`)
    ...
    describe("1 user, 2 messages", () => {
        ...
        before(async () => {
            ...
            expect(poll.messageTree.root.toString()).to.be.eq(
                poll.messageAq.getRoot(treeDepths.messageTreeDepth).toString()
            );
        });
        ...
        it("should produce the correct state root and ballot root", async () => {
            ...
            expect(newStateRoot.toString()).not.to.be.eq(
                currentStateRoot.toString()
            );
            expect(newBallotRoot.toString()).not.to.be.eq(
                currentBallotRoot.toString()
            );
            ...
            const hasherWitness = await hasherCircuit.calculateWitness(
                hasherCircuitInputs,
                true
            );
            await circuit.checkConstraints(witness);
            const hash = await getSignal(hasherCircuit, hasherWitness, "hash");
            expect(hash.toString()).to.be.eq(
                generatedInputs.inputHash.toString()
            );
        });
    });
