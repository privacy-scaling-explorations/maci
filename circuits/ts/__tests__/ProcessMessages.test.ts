...
describe("ProcessMessage circuit", function () {
    ...
    before(async () => {
        const circuitPath = path.join(
            __dirname,
            "../../circom/test",
            `processMessages_test.circom`
        );
        circuit = await tester(circuitPath);
        const hasherCircuitPath = path.join(
            __dirname,
            "../../circom/test",
            `processMessagesInputHasher_test.circom`
        );
    ...
    describe("1 user, 2 messages", () => {
        ...
        it("should produce the correct state root and ballot root", async () => {
            ...
            fs.writeFileSync("input.json", JSON.stringify(generatedInputs));
