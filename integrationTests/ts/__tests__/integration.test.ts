...
describe("integration tests", function () {
    ...
    afterEach(() => {
        if (existsSync(join(__dirname, "../../../cli/tally.json")))
            unlinkSync(join(__dirname, "../../../cli/tally.json"));
    ...
    const data = JSON.parse(
        readFileSync(join(__dirname, `./data/suites.json`)).toString()
    );
        ...
        it(testCase.description, async () => {
            ...
            for (let i = 0; i < users.length; i++) {
                ...
                for (let j = 0; j < users[i].votes.length; j++) {
                    const user = users[i];
                    const isKeyChange =
                        testCase.changeUsersKeys &&
                        j in testCase.changeUsersKeys[i];
                    ...
                    const newVoteWeight = isKeyChange
                        ? testCase.changeUsersKeys[i][j].voteWeight
                        : user.votes[j].voteWeight;
                    ...
                    const encPubKey = new PubKey(
                        genPubKey(encPrivKey.rawPrivKey)
                    );
            ...
            await genProofs(
                ...
                join(
                    __dirname,
                    "../../../cli/zkeys/TallyVotes_10-1-2_test.0.zkey"
                ),
                join(
                    __dirname,
                    "../../../cli/zkeys/ProcessMessages_10-2-1-2_test.0.zkey"
                ...
                join(
                    __dirname,
                    "../../../cli/zkeys/ProcessMessages_10-2-1-2_test"
                ),
                join(__dirname, "../../../cli/zkeys/TallyVotes_10-1-2_test"),
                join(
                    __dirname,
                    "../../../cli/zkeys/SubsidyPerBatch_10-1-2_test"
                ),
                ...
                join(
                    __dirname,
                    "../../../cli/zkeys/TallyVotes_10-1-2_test_js/TallyVotes_10-1-2_test.wasm"
                ),
                join(
                    __dirname,
            ...
            if (subsidyEnabled) {
                ...
                expectSubsidy(
                    maxMessages,
                    testCase.subsidy.expectedSubsidy,
                    subsidy
                );
            }
