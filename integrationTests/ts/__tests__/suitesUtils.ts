...
const executeSuite = async (data: any, expect: any) => {
  try {
    ...
    subsidyEnabled
      ? (subsidyZkeyFilePath = " --subsidy-zkey ./zkeys/SubsidyPerBatch_10-1-2_test.0.zkey")
      : (subsidyZkeyFilePath = "");
    subsidyEnabled
      ? (subsidyWitnessCalculatorPath = " --subsidy-witnessgen ./zkeys/SubsidyPerBatch_10-1-2_test")
      : (subsidyWitnessCalculatorPath = "");
    subsidyEnabled ? (subsidyResultFilePath = " --subsidy-file subsidy.json") : (subsidyResultFilePath = "");
    const genProofSubsidyArgument = subsidyResultFilePath + subsidyWitnessCalculatorPath + subsidyZkeyFilePath;
    ...
    for (let i = 0; i < users.length; i++) {
      ...
      const signupCommand =
        `node build/index.js signup` + ` -p ${userKeypair.pubKey.serialize()}` + ` -x ${maciAddress}`;
      execute(signupCommand);

      maciState.signUp(userKeypair.pubKey, BigInt(config.constants.maci.initialVoiceCredits), BigInt(Date.now()));
    }
    ...
    for (let i = 0; i < users.length; i++) {
      ...
      for (let j = 0; j < users[i].votes.length; j++) {
        ...
        const newVoteWeight = isKeyChange ? data.changeUsersKeys[i][j].voteWeight : users[i].votes[j].voteWeight;
        const nonce = users[i].votes[j].nonce;
        const salt = "0x" + genRandomSalt().toString(16);
        const userPrivKey = isKeyChange ? users[i].changeKeypair() : userKeypairs[i].privKey;
    ...
    if (subsidyEnabled) {
      ...
      expectSubsidy(config.constants.maci.maxMessages, data.subsidy.expectedSubsidy, subsidy);
    }
