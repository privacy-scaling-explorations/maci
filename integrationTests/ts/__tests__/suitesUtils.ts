...
const executeSuite = async (data: any, expect: any) => {
    try {
        ...
        subsidyEnabled
            ? (subsidyZkeyFilePath =
                  " --subsidy-zkey ./zkeys/SubsidyPerBatch_10-1-2_test.0.zkey")
            : (subsidyZkeyFilePath = "");
        subsidyEnabled
            ? (subsidyWitnessCalculatorPath =
                  " --subsidy-witnessgen ./zkeys/SubsidyPerBatch_10-1-2_test")
            : (subsidyWitnessCalculatorPath = "");
        subsidyEnabled
            ? (subsidyResultFilePath = " --subsidy-file subsidy.json")
            : (subsidyResultFilePath = "");
        const genProofSubsidyArgument =
            subsidyResultFilePath +
            subsidyWitnessCalculatorPath +
            subsidyZkeyFilePath;
        ...
        treeDepths.messageTreeSubDepth =
            config.constants.poll.messageBatchDepth;
